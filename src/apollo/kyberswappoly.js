...
const APIURL = 'https://api.thegraph.com/subgraphs/name/dynamic-amm/dmm-exchange-matic'
...
const tokensQuery = `
  query {
    pools(where: {createdAtTimestamp_gte: ${backTime}, reserveUSD_gt: "1"}, first: 10,  orderBy: createdAtTimestamp, orderDirection: desc) {
      id
      token0 {
        id
        name
...
function KyberswapPoly(obj) {
  ...
  useEffect(async () => {
   ...
   if(obj.view){
       viewPools = <> <a href='#' className='see-more'> See More  </a> </>
   }
        ...
        <>
                    <div className="board-col col2 pancakeswap">
                        <div className="board-block hot-pairs">
                            <div className="col-title"><span className="icon icon-home-pancakeswap"><img src="/images/kyber.jpg" alt="" /></span> KYBERSWAP <strong>NEW PAIRS</strong><img className="line" src="/images/line.svg" /> {viewPools}</div>
